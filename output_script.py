...
# Load your data (assuming 'dtsample' is a DataFrame)
# dtsample = pd.read_csv('your_data.csv')  # Example of loading data

# Ensure the 'Date' column is in datetime format
dtsample['Date'] = pd.to_datetime(dtsample['Date'], errors='coerce')  # Convert to datetime
dtsample['Date'] = pd.to_datetime(dtsample['Date'], errors='coerce')  # Convert 'Date' to datetime

# Step 2: Group by month and calculate the sum of 'Value'
monthly_value = dtsample.groupby(dtsample['Date'].dt.to_period('M'))['Value'].sum()
# Group by the monthly period and sum the 'Value' column
monthly_value = dtsample.groupby(dtsample['Date'].dt.to_period('M'))['Value'].sum()
